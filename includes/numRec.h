...
/*
//A doubleRepMantisa implementation of brent cause return type function overloading is forbidden in c++
template <typename regF>
doubleRepMantisa brentDoubleRep(doubleRepMantisa ax, doubleRepMantisa bx, doubleRepMantisa cx, regF f, doubleRepMantisa tol,
	      MDOUBLE *xmin) {

    const int ITMAX  = 100;
    const doubleRepMantisa CGOLD(0.3819660f);
    const doubleRepMantisa ZEPS(1.0e-10f);
	doubleRepMantisa minusOne(-1.0);
    int iter;
    doubleRepMantisa fu,fv,fw,fx,a,b,etemp,p,q,r,u,v,w,x;
	doubleRepMantisa d(0.0);
    doubleRepMantisa e(0.0);
	doubleRepMantisa half(0.5);
	doubleRepMantisa two(2.0);
	doubleRepMantisa zero(0.0);
	...
	LOG(10,<<"brent, f("<<x<<")="<<fx<<endl);
    for (iter=1;iter<=ITMAX;iter++) {
	doubleRepMantisa xm(0.5*convert(a+b));
	doubleRepMantisa tol1(convert(tol)*fabs(convert(x)));
	doubleRepMantisa tol2(2.0*convert((tol1+ZEPS)));
	...
	if (fabs(convert(e)) > convert(tol1)) {
		...
		doubleRepMantisa newQ(fabs(convert(q)));
	    q=newQ;
	    etemp=e;
	    e=d;
		...
		if (u+minusOne*a < tol2 || b+minusOne*u < tol2){
			doubleRepMantisa newD(SIGN(convert(tol1),convert(xm+minusOne*x)));
		    d=newD;
		}
	    }
	} else {
...
/*
//A doubleRepMantisa implementation of dbrent cause return type function overloading is forbidden in c++
template <typename regF, typename dF>
doubleRepMantisa dbrentDoubleRep(doubleRepMantisa ax, doubleRepMantisa bx, doubleRepMantisa cx, regF f,
	       dF df, doubleRepMantisa tol, MDOUBLE *xmin) {

    int iter,ok1,ok2;
    doubleRepMantisa a,b,d1,d2;
	doubleRepMantisa d(0.0);
	doubleRepMantisa e(0.0);
    doubleRepMantisa olde,u,u1,u2,v,w,x,xm;
	doubleRepMantisa fu,fv,fw,fx,du,dv,dw,dx;
	doubleRepMantisa minusOne(-1.0);
	doubleRepMantisa half(0.5);
	doubleRepMantisa two(2.0);
	doubleRepMantisa zero(0.0);
	...
	if (fabs(convert(e)) > convert(tol1)) { 
		...
		if (fabs(convert(d)) <= fabs(convert(half*olde))) {
		    u=x+d;
			if (u+minusOne*a < tol2 || b+minusOne*u < tol2){
				doubleRepMantisa sign(SIGN(convert(tol1),convert(xm+minusOne*x)));
				d=sign;
			}
		} else {
	...
	if (fabs(convert(d)) >= convert(tol1)) {
		...
		doubleRepMantisa sign(SIGN(convert(tol1),convert(d)));
	    u=x+sign; 
	    if (u<ax) u=x; // MY LATEST ADDITION!
	    fu=f(convert(u));
	    if (fu > fx) {
