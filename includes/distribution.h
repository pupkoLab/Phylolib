...
public:
	...
	virtual const size_t categories() const=0;  // @@@@ there is no need to return a const int.
	virtual void change_number_of_categories(int in_number_of_categories);
	virtual const MDOUBLE rates(const size_t i) const=0; // @@@@ there is no need to return a const MDOUBLE.
	virtual const MDOUBLE ratesProb(const size_t i) const=0; // @@@@ there is no need to return a const MDOUBLE.
