...
// A number in this representation is presented as x = _mantissa * 2^_expon
// Note: Base that the base is 2.
// As a standard, we set: 0.5<= _mantissa <1.0

class doubleRepMantisa{
public:
	doubleRepMantisa(){}; // empty constructor
...
private:
	void fixParams(); //This function set the mantisa to be between 0.5 and 1.0.
...
inline ostream &operator<<(ostream &out, const vector<doubleRepMantisa> &v){
...
inline ostream &operator<<(ostream &out, const vector<vector<doubleRepMantisa> > &m){
