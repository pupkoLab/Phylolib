...
optimizeSelectonParameters::optimizeSelectonParameters(tree& et, //find Best params and best BBL
   ...
   //initialization	
	...
	_bestL = likelihoodComputation2Codon::getTreeLikelihoodAllPosAlphTheSame(et,sc,sctm,spVec,distr);;
	MDOUBLE newL = _bestL;
...
//ALPHA (beta or gamma distribution parameter)
		if (!isAlphaSet){
			...
			newL = -brent(lowerValueOfParamAlpha,
						...
						upperBoundOnAlpha,
						evalParam(et,sc, sctm,spVec,-1,distr,isGamma),epsilonAlphaOptimization,&alphaFound);
			...
			if (newL > _bestL+epsilonLikelihoodImprovment ) {// update of likelihood ,v and model.
				...
				for (size_t categor = 0; categor < spVec.size();categor++)
...
//BETA (beta distribution parameter)
		if (!isBetaSet) {
			...
			newL = -brent(lowerValueOfParamBeta,
						...
						upperBoundOnBeta,
						evalParam(et,sc,sctm,spVec,-2,distr,isGamma),epsilonAlphaOptimization,&betaFound); 
...
//K parameter
		if (!isKappaSet){
			...
			newL = -brent(lowerValueOfParamK,   //optimaize Tr
					...
					upperValueOfParamK,
					evalParam(et,sc,sctm,spVec,0,distr,isGamma),epsilonKOptimization,&kFound); 
		...
		if (isGamma==false && !isBetaProbSet){ //optimize  beta probs
			if (!isOmegaSet){ // optimize omega  (M8 or M8b)
				...
				newL = -brent(omegaLowerBoundary, 
						...
						omegaUpperBoundary,
						evalParam(et,sc,sctm,spVec,1,distr,isGamma),0.01,&omegaFound); 
			...
			newL = -brent(0.0,initialGuessValueOfParamBetaProb,1.0,
					evalParam(et,sc,sctm,spVec,2,distr,isGamma),0.01,&betaProbFound); 
...
evalParam::evalParam(const evalParam &other): _et(other._et),_sc(other._sc),_sctm(other._sctm),
...
MDOUBLE evalParam::operator()(MDOUBLE param){
	...
	MDOUBLE res = likelihoodComputation2Codon::getTreeLikelihoodAllPosAlphTheSame(_et,_sc,_sctm,_spVec,_distr);
...
void evalParam::updateBeta(MDOUBLE param){
	...
	for (size_t categor = 0; categor < _spVec.size();categor++){
