...
//constructors:
	...
	//questions only:
	const size_t seqLen() const {return _seqDataVec.empty()? 0 : _seqDataVec[0].seqLen();}
	const size_t numberOfSeqs() const {return _seqDataVec.size();}
	const size_t alphabetSize() const {return _seqDataVec.empty()? 0 : _seqDataVec[0].getAlphabet()->size();}
	...
	const vector<size_t> getAlphabetDistribution() const;
	vector<string> getSeqNamesThatMatchPos(const int pos, const char charId);
	const vector<size_t> getAlphabetDistribution(size_t pos) const;
...
//make changes:
	...
	void remove(const size_t idSeq);
	void removeAll();
