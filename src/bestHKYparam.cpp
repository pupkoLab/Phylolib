...
//	brLenOpt br1(*et,*pi,weights);
	...
	for (int i=0; i < maxTotalIterations; ++i) {
		...
		bblEM bblEM1(et,sc,sctm,sp,NULL,maxBBLIterations,epsilonLikelihoodImprovment);//maxIterations=1000
...

{
	...
	for (int i=0; i < maxTotalIterations; ++i) {
		...
		bblEM bblEM1(et,sc,sctm,sp,NULL,maxBBLIterations,epsilonBBL);
...

{
	...
	//doubleRepMantisa epsilonGlobalAlphaOptimizationDR(epsilonGlobalAlphaOptimization);//DR
	Vdouble newLvec;
	newLvec.resize(msp->getSPVecSize());
	//doubleRepMantisa oldL(VERYSMALL);//DR
	//doubleRepMantisa newL;
	MDOUBLE oldL = VERYSMALL;
	MDOUBLE newL;
	...
	for (int i=0; i < maxTotalIterations; ++i) {
		if(optimizeLocalParams){
			...
			LOG(2,<<"Done with HKY local params optimization. LL: "<<sumVdouble(_bestLvec)<<endl);
			LOG(2,<<"Local Params:"<<endl);
			LOG(2,<<"HHY:");
			for(spIndex = 0;spIndex < _bestHkyParamVec.size();++spIndex){
				LOG(2,<<_bestHkyParamVec[spIndex]<<",";);
			}
			LOG(2,<<endl);
			LOG(2,<<"local alpha:");
			...
			LOG(2,<<endl);
			_bestLvec = likelihoodComputation::getTreeLikelihoodProportionalAllPosAlphTheSame(et,sc,msp,pProportionDist,weights);
			LOG(2,<<"LL*: "<<sumVdouble(_bestLvec)<<endl);
