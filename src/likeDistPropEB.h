...
public:
	MDOUBLE operator() (MDOUBLE dist) {
		...
		MDOUBLE sumDL = 0.0;
		for (size_t gene=0; gene < _msp->getSPVecSize(); ++gene) {
			for (size_t alph1=0; alph1 <  _ctc[gene][_nodeID].alphabetSize(); ++alph1){
				for (size_t alph2=0; alph2 <  _ctc[gene][_nodeID].alphabetSize(); ++alph2){
					for(size_t globalRateCategor = 0;globalRateCategor < _pProportionDist->categories();++globalRateCategor){
						...
						for (size_t localRateCategor = 0; localRateCategor < _msp->getSp(gene)->categories(); ++localRateCategor) {
