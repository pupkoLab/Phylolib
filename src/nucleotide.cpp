...
nucleotide::nucleotide() {
	_relation.resize(4);
	for (size_t i=0; i < _relation.size(); ++i) {
...
size_t nucleotide::fromChar(const char s) const {
	switch (s) {
		...
		case '-' : case'_' : return 16; break;//gap
		case 'x' : case'X' : return 15; break;
		case '.' : return 17; break;		// . is used in some sequence files as the character just in the line above...
		default:
		vector<string> err;
