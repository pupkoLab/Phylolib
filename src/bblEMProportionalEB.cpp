...
Vdouble bblEMProportionalEB::compute_bblEMPropEB(
			...
			const MDOUBLE tollForPairwiseDist,
			const MDOUBLE* likelihoodLast){
	LOG(5,<<"Allocating place"<<endl);
	allocatePlacePropEB();
	LOG(5,<<"Done Allocating place"<<endl);
	Vdouble oldLvec(_numberOfGenes,VERYSMALL);
	Vdouble currLvec;
	currLvec.resize(_numberOfGenes);
	tree oldT = _et;
	//doubleRepMantisa epsilonDR(epsilon);//DR
	for (int i=0; i < maxIterations; ++i) {
		LOG(5,<<"Calling computeUpPropEB on iteration "<<i<<endl);
		computeUpPropEB();
		for (size_t geneN=0; geneN < _numberOfGenes; ++geneN) {
		...
		LOG(5,<<"--- Iter="<<i<<" logL="<<sumVdouble(currLvec)<<endl);
		if(sumVdouble(oldLvec)<=sumVdouble(currLvec)){	// make sure not to use tree with lower likelihood then last computed likelihood (before BBL-EM)
			LOG(4,<<"Likelihood improved. oldL = "<<sumVdouble(oldLvec)<<" newL = "<<sumVdouble(currLvec)<<endl);
			if(likelihoodLast){
				if(*likelihoodLast<=sumVdouble(currLvec))
					oldT = _et;	 // L didn't go down
				else
					LOG(4,<<"Likelihood went down compared pre-BBL oldL="<<*likelihoodLast<<" newL="<<sumVdouble(currLvec)<<" Do not update tree"<<endl);
			}
			else{
				oldT = _et;	 // L didn't go down
				LOG(7,<<"Tree Updated"<<endl);
			}
		}
		else 
			LOG(4,<<"Likelihood did not improve. oldL="<<sumVdouble(oldLvec)<<" newL="<<sumVdouble(currLvec)<<" Do not update tree"<<endl);
		...
		if (sumVdouble(currLvec) < sumVdouble(oldLvec) + epsilon) { // need to break
			if (sumVdouble(currLvec)<sumVdouble(oldLvec)) {
				...
				LOG(4,<<"Finished bblEMPropEB. Likelihood ="<<sumVdouble(oldLvec)<<endl);
				return oldLvec; // keep the old tree, and old likelihood
			} else {
                //update the tree and likelihood and return
				LOG(4,<<"Finished bblEMPropEB. Likelihood ="<<sumVdouble(currLvec)<<endl);
				return currLvec;
			}
		}
	...
	if (sumVdouble(currLvec)<sumVdouble(oldLvec)) {
		_et = oldT;
		LOG(4,<<"Finished bblEMPropEB max iter. Likelihood ="<<sumVdouble(oldLvec)<<endl);
		return oldLvec; // keep the old tree, and old likelihood
	} 
	else{
		LOG(4,<<"Finished bblEMPropEB max iter. Likelihood ="<<sumVdouble(currLvec)<<endl);
        return currLvec;
	}
}
...
void bblEMProportionalEB::allocatePlacePropEB() {
	...
	for (size_t geneN=0; geneN < _numberOfGenes; ++geneN) {
		...
		for(size_t pos = 0;pos < _sc[geneN].seqLen();++pos){
		...
		for (size_t i=0; i < _computeCountsV[geneN].size(); ++i) {
...
void bblEMProportionalEB::computeUpPropEB(){
	for (size_t geneN=0; geneN < _numberOfGenes; ++geneN) {
		for(size_t globalRateCategor = 0;globalRateCategor < _pProportionDist->categories();++globalRateCategor){
			...
			computeUpAlg cupAlg;
			seqContainerTreeMap sctm(_sc[geneN], _et);
			for (size_t pos=0; pos < _sc[geneN].seqLen(); ++pos) {
				for (size_t localRateCategor = 0; localRateCategor < _msp->getSp(geneN)->categories(); ++localRateCategor) {
					cupAlg.fillComputeUp(_et,_sc[geneN],pos,_pij[geneN][globalRateCategor][localRateCategor], sctm,_cup[geneN][pos][globalRateCategor][localRateCategor]);
				}
			}
		}
	}
...
void bblEMProportionalEB::bblEM_itPropEB(const MDOUBLE tollForPairwiseDist){
	for (size_t geneN=0; geneN < _numberOfGenes; ++geneN) {
		for (size_t treeNode=0; treeNode < _computeCountsV[geneN].size(); ++treeNode) {
...
void bblEMProportionalEB::addCountsPropEB(const int gene,const int pos, tree::nodeP mynode, const vector<doubleRepMantisa> & posProb, const MDOUBLE weig){
	computeCounts cc;
	stochasticProcess * sp = _msp->getSp(gene);
	for (size_t globalRateCategor =0; globalRateCategor< _pProportionDist->categories(); ++globalRateCategor) {
		for (size_t localRateCategor =0; localRateCategor < sp->categories(); ++localRateCategor) {
