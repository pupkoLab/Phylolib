...
public:  
	...
	size_t alphabetSize() const {return _countValues.size();}
	void zero();
	...
	size_t getSize() const {return _countValues.size();}
...
class countTableComponentGam{ // This is for a specific node.
public:  
	...
	size_t alphabetSize() const {return _countValues.empty()?0:_countValues[0].alphabetSize();}
	void zero(){
		for (size_t rateCat=0; rateCat < _countValues.size(); ++rateCat) _countValues[rateCat].zero();
	}
	...
	void countTableComponentAllocatePlace(const int alphabetSize,
		...
		for (size_t rateCat=0; rateCat < _countValues.size(); ++rateCat){
...
public:  
	...
	size_t alphabetSize() const {return _countValues.empty()?0:_countValues[0].alphabetSize();}
	void zero(){
		for (size_t globalRateCat=0; globalRateCat < _countValues.size(); ++globalRateCat) _countValues[globalRateCat].zero();
	}
	...
	void countTableComponentAllocatePlace(const int alphabetSize,
		...
		for(size_t globalRateCat = 0;globalRateCat < _countValues.size(); ++globalRateCat){
