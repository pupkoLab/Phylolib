...
public:
	explicit uniformDistribution(const size_t numOfCategories, MDOUBLE lowerBound,
		MDOUBLE upperBound);
	...
	MDOUBLE getBorder(const size_t i) const ; 	//return the ith border. Note:  _bonderi[0] = m_lowerLimit, _bondery[categories()] = m_upperLimit

	void setUniformParameters(const size_t numOfCategories, MDOUBLE lowerBound, MDOUBLE upperBound);
