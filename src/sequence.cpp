...
: _alphabet(inAlph->clone()), _remark(other._remark), _name(other._name), _id(other._id)
{
	...
	if ( (other._alphabet->size() == 20 && inAlph->size() == 2) //A.M. why are we comparing sizes? we can create a string that will contain alphabet name. it's more elegant and clear.
		|| (other._alphabet->size() == 4 && inAlph->size() == 2) )
	{
		for (size_t k=0; k < other.seqLen() ;k += other._alphabet->stringSize())
		{
	...
	{
		for (size_t k=0; k < other.seqLen() ;++k)
		{
			int charId = other._vec[k];
...
class particip { //A.M. What's that?
public:
	explicit particip()  {}
	bool operator()(int i) {
		return (i==-1000);
...
size_t sequence::seqLenSpecific() const
{
	size_t res = 0;
