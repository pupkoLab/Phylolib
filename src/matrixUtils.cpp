...
void printMatrix(const VVdouble &mat, ostream &out) {
	...
	for (size_t row=0; row<num; row++) {
		for (size_t position=0; position<mat[row].size(); position++) {
...
void resize_VVVV(int dim1, int dim2, int dim3, int dim4,  VVVVdouble& vetor){
	...
	vetor.resize(dim1);
	for (size_t posNum=0;posNum<vetor.size();++posNum){
		vetor[posNum].resize(dim2);
		for (size_t n=0;n<vetor[posNum].size();++n){
