...
vector<tree> eliminateHalf(vector<tree>& tVec,
	...
	for (size_t i=0; i < tVec.size(); ++i) {
		seqContainerTreeMap sctm(orginal,tVec[i]);
		bblEM bblEM1(tVec[i],orginal, sctm,sp,NULL,maxIterEM,0.01);
...
//	}
	...
	for (size_t p=0; p < sortedL.size(); ++p ){
...
tree getBestMLTreeFromManyNJtrees(sequenceContainer & allTogether,
	...
	while (tVec.size() < numOfNJtrees) {
		...
		for (size_t p=0; p < startingTreeWeights.size(); ++p){
...
//------------------ chossing the ML tree from these NJ trees --------------------
	...
	seqContainerTreeMap sctm(allTogether, tVec[0]);
	bblEM bblEM1(tVec[0],allTogether, sctm,sp,NULL,100,0.01);
