...
//	brLenOpt br1(*et,*pi,weights);
	...
	//doubleRepMantisa epsilonLoglikelihoodForGlobalRateAlphaOptimizationDR(epsilonLoglikelihoodForGlobalRateAlphaOptimization);//DR
	string alphas;
	...
	for(i=0; i < maxTotalIterations; ++i) {
		...
		if(optimizeLocalAlpha){
			...
			LOG(2,<<"Done with local alpha optimization"<<endl<<"LL:"<<sumVdouble(_bestLvec)<<endl);
			LOG(2,<<"Local Alphas:");
		...
		if(optimizeGlobalAlpha){
			//doubleRepMantisa b_globalAlpha_x(_bestGlobalAlpha);//DR
            //doubleRepMantisa a_globalAlpha_x(0.0);//DR
			//doubleRepMantisa c_globalAlpha_x(upperBoundOnGlobalRateAlpha);//DR
		...
		if(optimizeTree){
			if(branchLengthOptimizationMethod == "bblLS"){
				...
				LOG(2,<<"Done with bblEM"<<endl<<"LL:"<<sumVdouble(_bestLvec)<<endl);
			}
		...
		LOG(2,<<"Done with optimization iteration "<<i<<". LL: "<<sumVdouble(_bestLvec)<<endl);
	}
}
