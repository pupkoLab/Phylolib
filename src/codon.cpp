...
void codonUtility::initSubMatrices(const codon& cod){
	...
	for (size_t i = 0; i < _trtvDiff.size(); ++i)
	{
...
void codonUtility::getCodonUsage(const sequenceContainer& sc, Vint& codonCounter, Vdouble& codonUsage)
{
	...
	for (size_t c = 0; c < codonCounter.size(); ++c)
...
{	
	...
	for (size_t pos = 0; pos < sc.seqLen(); ++pos)
	{
		...
		for (size_t s = 0; s < sc.numberOfSeqs();++s)
		{
			int id = sc.placeToId(s);
			int cod = sc[id][pos];
			if(!alph.isSpecific(cod))
...
bool codon::isStopCodon(const size_t in_id) const
{
