								 ...
								 unObservableData* unObservableData_p)
{
	...
	vector<doubleRepMantisa> pGivenR(sp.categories(),0.0);
	doubleRepMantisa sum=0;
	doubleRepMantisa LofPos_givenRateCat;
	LOG(8,<<pos+1<<"\t"); //DEBUG
	for (size_t cat=0; cat < sp.categories(); ++cat) {
		LofPos_givenRateCat = likelihoodComputation::getLofPos(pos,et,sc, sctm,cpg[cat],sp);
...
// ver2 - fix likelihoodForEachCat by LforMissingDataAll
	...
	doubleRepMantisa sumOfSquares(0.0);
	doubleRepMantisa bestRate_dblRep(0.0);
	
	LOG(6,<<"Pos "<<pos<<" content = "<<sc[0][pos]<<" ,total likelihood = "<<sum<<endl); //DEBUG
	
	for (size_t j=0; j < sp.categories(); ++j) {
...
// ver3 - fix likelihoodForEachCat after multiplied by Prob - Error??
		...
		doubleRepMantisa tmp = pGivenR[j]*sp.rates(j);
	...
	if (varRate<-tolerance)
		LOG(3,<<"Error in computeEB_EXP_siteSpecificRate pos="<<pos<<", varRate="<<varRate<<" (< 0) \n");
										...
										VVdouble* LpostPerCat)	//2 fill (*LpostPerCat)[globalRate][localRate][pos]
{
	...
	for (size_t gene=0; gene < scVec.size(); ++gene) {
												...
												VVdouble* LpostPerCat)	
{
	seqContainerTreeMap sctm(sc, et);
	// here we compute P(r | data)
	vector<doubleRepMantisa> pGivenR(pProportionDist->categories(),0.0);
	doubleRepMantisa sum=0;
	doubleRepMantisa LofGene_givenRateCat = 0.0;
	LOG(8,<<gene+1<<"\t"); //DEBUG
	for (size_t cat=0; cat < pProportionDist->categories(); ++cat) {
	...
	for (size_t j=0; j < pProportionDist->categories(); ++j) {
		pGivenR[j]/=sum; // So that pGivenR is probability.
		                 // From here on we can convert it back
		                 // to MDOUBLE because it's not a very
		                 // small likelihood any more
		...
		doubleRepMantisa tmp = pGivenR[j]*pProportionDist->rates(j);
	...
	MDOUBLE tolerance = 0.0001; // tolerance for variance is not very exact, and also exact computation not very important
	if (varRate<-tolerance)
		LOG(3,<<"Error in computeEB_EXP_siteSpecificRateProportional gene="<<gene<<", varRate="<<varRate<<" (< 0) \n");
	if ((varRate<0) && (varRate>=-tolerance))
	    varRate = 0;
	stdRate = sqrt(varRate);
...
void computeEB_EXP_siteSpecificRate(Vdouble & ratesV,
	...
	lowerBoundV.resize(sc.seqLen());
	upperBoundV.resize(sc.seqLen());
	for (size_t treeNum=0; treeNum<etVec.size(); ++treeNum) {
		for (size_t spNum = 0; spNum<spVec.size(); ++spNum) {
            computePijGam cpg;
