...
class suffStatSpecHomPos{ // this is for a specific node.
	public:
		void set(const int letter,const doubleRepMantisa& val) {
			_V[letter]=val;
		}
		
		doubleRepMantisa get(const int letter) const	{
			doubleRepMantisa tmp=_V[letter];
//		    cout << "tmp =";
//		    tmp.outputn(cout);
		...
		void allocatePlace(const size_t alphabetSize) {
	...
	private:
		vector<doubleRepMantisa> _V;//size = letter
};
//-------------------------------------------------------------
/*
...
class suffStatGlobalHomPos{ // this is for all nodes
	public:
		void set(const int nodeId,const int letter,const doubleRepMantisa val) {
			_V[nodeId].set(letter,val);
		}
		
		doubleRepMantisa get(const int nodeId,const int letter) const	{
			doubleRepMantisa tmp(_V[nodeId].get(letter));
//		    tmp;
...
//		    tmp.outputn(cout);
		...
		void allocatePlace(const size_t numOnNodes,const size_t alphabetSize) {
			_V.resize(numOnNodes);
			for (size_t i=0;i<_V.size();++i) {_V[i].allocatePlace(alphabetSize);}
		}
		bool isEmpty (){return (_V.empty());};
		size_t size() const {return _V.size();}
	private:
...
class suffStatGlobalGamPos{ // this is for all nodes
	public:
		void set(const int categor,const int nodeId,const int letter,const doubleRepMantisa val) {
			_V[categor].set(nodeId,letter,val);
		}
		
		doubleRepMantisa get(const int categor,const int nodeId,const int letter) const	{
			return _V[categor].get(nodeId,letter);
		}

		void allocatePlace(const size_t categor,const size_t numOnNodes,const size_t alphabetSize) {
			_V.resize(categor);
			for (size_t i=0;i<_V.size();++i) {_V[i].allocatePlace(numOnNodes,alphabetSize);}
		}
		bool isEmpty (){return (_V.empty());}
		size_t size() const {return _V.size();}
...
class suffStatGlobalGamProportionalPos{ // this is for all nodes
	public:
		void set(const int globalRateCategor,const int localRateCategor,const int nodeId,const int letter,const doubleRepMantisa val) {
			_V[globalRateCategor].set(localRateCategor,nodeId,letter,val);
		}
		
		doubleRepMantisa get(const int globalRateCategor,const int localRateCategor,const int nodeId,const int letter) const	{
		...
		void allocatePlace(const int globalRateCategor,const int localRateCategor,const int numOnNodes,const int alphabetSize) {
			...
			for (size_t i=0;i<_V.size();++i) {_V[i].allocatePlace(localRateCategor,numOnNodes,alphabetSize);}
		}
		bool isEmpty (){return (_V.empty());}
		size_t size() const {return _V.size();}
...
class suffStatGlobalGam{ // this is for all positions (and for all nodes).
	public:
		void set(const int pos,const int categor,const int nodeId,const int letter,const doubleRepMantisa val) {
			_V[pos].set(categor,nodeId,letter,val);
		}
		
		doubleRepMantisa get(const int pos,const int categor,const int nodeId,const int letter) const	{
		...
		void allocatePlace(const int pos,const int categor,const int numOnNodes,const int alphabetSize) {
			_V.resize(pos);
			for (size_t i=0;i<_V.size();++i) {_V[i].allocatePlace(categor,numOnNodes,alphabetSize);}
		}
		bool isEmpty (){return (_V.empty());}
		size_t size() const {return _V.size();}
	suffStatGlobalGamPos& operator[] (int index) {return _V[index];}
...
class suffStatGlobalGamProportional{ // this is for all positions (and for all nodes).
	public:
		void set(const int pos,const int globalRateCategor,const int localRateCategor,const int nodeId,const int letter,const doubleRepMantisa val) {
			_V[pos].set(globalRateCategor,localRateCategor,nodeId,letter,val);
		}
		
		doubleRepMantisa get(const int pos,const int globalRateCategor,const int localRateCategor,const int nodeId,const int letter) const	{
		...
		void allocatePlace(const int pos,const int globalRateCategor,const int localRateCategor,const int numOnNodes,const int alphabetSize) {
			_V.resize(pos);
			for (size_t i=0;i<_V.size();++i) {_V[i].allocatePlace(globalRateCategor,localRateCategor,numOnNodes,alphabetSize);}
		}
		bool isEmpty (){return (_V.empty());}
		size_t size() const {return _V.size();}
...
class suffStatGlobalHom{ // this is for all positions (and for all nodes).
	public:
		void set(const int pos, const int nodeId, const int letter,const doubleRepMantisa val) {
			_V[pos].set(nodeId, letter, val);
		}
		
		doubleRepMantisa get(const int pos, const int nodeId, const int letter) const {
