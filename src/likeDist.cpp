...
MDOUBLE likeDist::evalLikelihoodForDistance(const stochasticProcess& sp,
    ...
    for (size_t pos=0; pos < s1.seqLen(); ++pos){
	...
	if (s1.isUnknown(pos) && s2.isSpecific(pos)) { 
	    ...
	    for (size_t rateCategor = 0; rateCategor<sp.categories(); ++rateCategor) {
		...
		if (s1.isSpecific(pos) && s2.isSpecific(pos)) {//simple case, where AA i is changing to AA j
		    ...
		    for (size_t iS1 =0; iS1< sp.alphabetSize(); ++iS1) {
			for (size_t iS2 =0; iS2< sp.alphabetSize(); ++iS2) {
...
public:
	...
	for (size_t pos=0; pos < _s1.seqLen(); ++pos){
		...
		for (size_t rateCategor = 0; rateCategor<_sp.categories(); ++rateCategor) {
			...
			for (size_t iS1 =0; iS1< _sp.alphabetSize(); ++iS1) {
			    for (size_t iS2 =0; iS2< _sp.alphabetSize(); ++iS2) {
